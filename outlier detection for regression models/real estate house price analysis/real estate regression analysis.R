...
altitude_data <- select(dd, `X6 longitude`, `X5 latitude`)
...
hlm2 <- lm(`Y house price of unit area`~
             ...
             `X4 number of convenience stores`, data=newdd)
...
lev_out2 <- filter(lev_out, 
                   studentizedresiuals > 3 & leverage > 0.01207729 |
                     studentizedresiuals < (-3) & leverage > 0.01207729)
...
# improved model
hlm2_lev_out <- lm(`Y house price of unit area`~
             `X2 house age`+`X3 distance to the nearest MRT station`+
